...
def main():
  ...
  # Command line parser
  parser = argparse.ArgumentParser(description='Launch E3SM post-processing tasks', usage="zppy -c <config>")
  parser.add_argument(
  ...
  if tmp.startswith('compy'):
      machine = "compy"
      environment_commands = 'source /share/apps/E3SM/conda_envs/load_{}_e3sm_unified.sh'.format(config['default']['e3sm_unified'])
  elif tmp.startswith('cori'):
      machine = "cori"
      environment_commands = 'source /global/cfs/cdirs/e3sm/software/anaconda_envs/load_{}_e3sm_unified.sh'.format(config['default']['e3sm_unified'])
  elif tmp.startswith('blues'):
      machine = "anvil"
      environment_commands = 'source /lcrc/soft/climate/e3sm-unified/load_{}_e3sm_unified.sh'.format(config['default']['e3sm_unified'])
