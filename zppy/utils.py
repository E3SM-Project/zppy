...
def getTasks(config, section_name):
    ...
    if (len(sub_section_names) == 0):
        ...
        if task['active']: tasks.append(task)
...
def getComponent(input_files):
    ...
    if tmp in ('cam', 'eam'):
        ...
        raise Exception('Cannot extract component name from input_files %s' % (input_files))
...
def submitScript(scriptFile, dependFiles=[], export='ALL'):
    ...
    for dependFile in dependFiles:
        if os.path.isfile(dependFile):
            ...
            print("...skipping because of dependency status file missing\n   %s" % (dependFile))
            return -1
    ...
    if len(dependIds) == 0:
        ...
        for i in dependIds:
            jobs += ":{:d}".format(i)
        command = "sbatch --export=%s --dependency=afterok%s %s" % (export,jobs,scriptFile)
