...
- ``update_bash_generation_expected_files_${machine_name}.sh`` and
  ``update_defaults_expected_files_${machine_name}.sh`` each only produce a single
  expected-files directory, so there's nothing to partially select within them --
  simply skip running that script if you don't want to touch those results yet.
- Regardless of whether you're doing a full or partial update, **always** run the
  full backup step first (``cp -r ${expected_results_dir} ${expected_results_records_dir}/expected_results_until_yyyymmdd``).
  This keeps a complete snapshot to diff or revert against, even though the update
  itself may only touch part of the tree.
- After a partial update, ``image_list_expected_${test_name}.txt`` is still
  regenerated by walking the whole ``expected_${test_name}`` directory, so it
  correctly reflects a mix of freshly-updated and previously-existing diag results.
