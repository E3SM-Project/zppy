...
def global_time_series(config, scriptDir):
    ...
    for c in tasks:
        ...
        for s in year_sets:
            ...
            # Load useful scripts
            c['global_time_series_dir'] = os.path.join(scriptDir, '{}_dir'.format(prefix))
            ...
            for yr in range(c['year1'], c['year2'], c['ts_num_years']):
                ...
                dependencies.append(os.path.join(scriptDir, 'ts_%s_%04d-%04d-%04d.status' % ('atm_monthly_glb',start_yr,end_yr,c['ts_num_years'])))
