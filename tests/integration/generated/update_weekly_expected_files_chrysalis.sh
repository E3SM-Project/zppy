...
do
    ...
    if [[ "${test_name,,}" =~ "v2" ]]; then
      ...
      cp -r /lcrc/group/e3sm/public_html/diagnostic_output/ac.forsyth2/zppy_weekly_${test_name}_www/test-zppy-gts-split-20250605-try4/v2.LR.historical_0201 /lcrc/group/e3sm/public_html/zppy_test_resources/expected_${test_name}
    else
      # We need the v3 case name
      cp -r /lcrc/group/e3sm/public_html/diagnostic_output/ac.forsyth2/zppy_weekly_${test_name}_www/test-zppy-gts-split-20250605-try4/v3.LR.historical_0051 /lcrc/group/e3sm/public_html/zppy_test_resources/expected_${test_name}
    fi
    ...
    if [[ "${test_name,,}" == "bundles" ]]; then
      ...
      cp -r /lcrc/group/e3sm/ac.forsyth2/zppy_weekly_bundles_output/test-zppy-gts-split-20250605-try4/v3.LR.historical_0051/post/scripts/bundle*.bash /lcrc/group/e3sm/public_html/zppy_test_resources/expected_bundles/bundle_files
    fi
