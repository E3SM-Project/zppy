...
do
    ...
    if [[ "${test_name,,}" == "comprehensive_v2" ]]; then
      cp -r /lcrc/group/e3sm/public_html/diagnostic_output/ac.forsyth2/zppy_weekly_${test_name}_www/test-346-20241209/v2.LR.historical_0201 /lcrc/group/e3sm/public_html/zppy_test_resources/expected_${test_name}
    else
      cp -r /lcrc/group/e3sm/public_html/diagnostic_output/ac.forsyth2/zppy_weekly_${test_name}_www/test-346-20241209/v3.LR.historical_0051 /lcrc/group/e3sm/public_html/zppy_test_resources/expected_${test_name}
    fi
    if [[ "${test_name,,}" == "bundles" ]]; then
      mkdir -p /lcrc/group/e3sm/public_html/zppy_test_resources/expected_bundles/bundle_files
      cp -r /lcrc/group/e3sm/ac.forsyth2/zppy_weekly_bundles_output/test-346-20241209/v3.LR.historical_0051/post/scripts/bundle*.bash /lcrc/group/e3sm/public_html/zppy_test_resources/expected_bundles/bundle_files
    fi
    zppy_top_level=$(pwd)
