...
def test_images(tester, test_name, case_name):
    ...
    check_mismatched_images(
        tester, actual_images_dir, expected_images_file, expected_images_dir, diff_dir
    )


# To run just one test:
# python -u -m unittest tests/integration/test_weekly.py TestWeekly.test_{name}
class TestWeekly(unittest.TestCase):
    def test_comprehensive_v2_images(self):
        test_images(self, "comprehensive_v2", V2_CASE_NAME)

    def test_comprehensive_v3_images(self):
        test_images(self, "comprehensive_v3", V3_CASE_NAME)

    def test_bundles_images(self):
        test_images(self, "bundles", V3_CASE_NAME)
