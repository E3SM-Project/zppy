...
for y in range(start_yr,end_yr,10):
  ...
  for year in range(year1, year2+1):
    print("year=",year)
    inFiles = '%s/*mpaso.hist.am.timeSeriesStatsMonthly.%04d-??-??.nc' % (path_in,year)
  ...
  for file in files:
    ...
    date_start = np.array([datetime.strptime(x[0:10], '%Y-%m-%d') for x in xtime_startMonthly])
    date_end   = np.array([datetime.strptime(x[0:10], '%Y-%m-%d') for x in xtime_endMonthly])
    ...
    sumLayerMaskValue = f.variables['timeMonthly_avg_avgValueWithinOceanLayerRegion_sumLayerMaskValue'][:,iregion,:]
    avgLayerArea = f.variables['timeMonthly_avg_avgValueWithinOceanLayerRegion_avgLayerArea'][:,iregion,:]
    avgLayerThickness = f.variables['timeMonthly_avg_avgValueWithinOceanLayerRegion_avgLayerThickness'][:,iregion,:]
    avgLayerTemperature = f.variables['timeMonthly_avg_avgValueWithinOceanLayerRegion_avgLayerTemperature'][:,iregion,:]
    ...
    for i in range(len(date_start)):
      print("Start, End, OHC = %s (%s), %s (%s), %e" % (date_start[i],tstart[i],date_end[i],tend[i],ohc_tot[i]))
